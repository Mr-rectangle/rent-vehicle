...
if (isset($_GET["id"])) {
	...
	$sql = "UPDATE `user` SET `is_delete` = '1' WHERE `user`.`id` = ".$_GET["id"]."";
	$conn->query($sql);
	$sql = "UPDATE `vehicle` SET `is_delete` = '1' WHERE  `vehicle`.`owner_id` = ".$_GET["id"]."";
	$conn->query($sql);
	$sql = "UPDATE `rent` SET `is_delete` = '1' WHERE `rent`.`user_id` = ".$_GET["id"]."";
